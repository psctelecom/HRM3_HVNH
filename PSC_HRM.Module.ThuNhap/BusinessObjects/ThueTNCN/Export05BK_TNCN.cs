    ...
    public class Export05BK_TNCN
    {
        public void XuLy(IObjectSpace obs, ToKhaiQuyetToanThueTNCN toKhai)
        {
            using (SaveFileDialog dialog = new SaveFileDialog())
            {
                ...
                dialog.FileName = "Bang_Ke_05BK.xls";
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    CriteriaOperator filter = CriteriaOperator.Parse("TargetTypeName=?", "PSC_HRM.Module.ThuNhap.Thue.BangKeThueTNCNNgoai");
                    ...
                    if (bieuMau != null)
                    {
                        ...
                        //Đưa dữ liệu ra
                        using (DataTable dt = DataProvider.GetDataTable("spd_TaiChinh_ThueTNCN_LayDanhSach05B_BK_TNCN", CommandType.StoredProcedure, new SqlParameter("@ToKhaiQuyetToanThueTNCN", toKhai.Oid)))
                        {
