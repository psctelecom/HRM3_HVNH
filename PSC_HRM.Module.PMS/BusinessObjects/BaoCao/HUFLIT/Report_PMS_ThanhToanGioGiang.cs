...
namespace PSC_HRM.Module.PMS.BaoCao
{
    [NonPersistent]
    [ImageName("BO_Report")]
    [ModelDefault("Caption", "Báo cáo: Bảng thanh toán giờ giảng dạy vượt chuẩn")]
    public class Report_PMS_ThanhToanGioGiang : StoreProcedureReport
    {
        ...
        public void UpdateBangChot()
        {
            if (NamHoc != null)
            {
                if (listbc != null)
                {
                    ...
                    {
                        if (item.HocKy.MaQuanLy.Equals("HK01"))
                            listbc.Add(item);
                    }
                }
            }
        ...
        public Report_PMS_ThanhToanGioGiang(Session session) : base(session) { }
