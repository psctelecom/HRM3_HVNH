...
{
    public class MailMerge_QuyetDinhChuyenNganhDaoTaoMoi : IMailMerge<IList<QuyetDinhChuyenNganhDaoTaoMoi>>
    {
        public void Merge(DevExpress.ExpressApp.IObjectSpace obs, IList<QuyetDinhChuyenNganhDaoTaoMoi> qdList)
        {
            var list = new List<Non_QuyetDinhChuyenNganhDaoTaoMoi>();
            Non_QuyetDinhChuyenNganhDaoTaoMoi qd;
            foreach (QuyetDinhChuyenNganhDaoTaoMoi quyetDinh in qdList)
            {
                qd = new Non_QuyetDinhChuyenNganhDaoTaoMoi();
                ...
                qd.NganhDaoTaoMoi = quyetDinh.NganhDaoTaoMoi != null ? quyetDinh.NganhDaoTaoMoi.TenNganhDaoTao : "";
            ...
            MailMergeTemplate merge = HamDungChung.GetTemplate(obs, "QuyetDinhChuyenNgahDaoTaoMoi.rtf");
            if (merge != null)
                MailMergeHelper.ShowEditor<Non_QuyetDinhChuyenNganhDaoTaoMoi>(list, obs, merge);
            else
                HamDungChung.ShowWarningMessage("Không tìm thấy mấu in QĐ chuyển ngành đào tạo trong hệ thống.");
        }
    }
}
