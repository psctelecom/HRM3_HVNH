...
namespace PSC_HRM.Module.BaoHiem
{
    ...
    [ModelDefault("Cation", "Đề nghị hưởng trợ cấp nghỉ DS-PHSK sau thai sản")]
    [Appearance("TroCap_PHSKSauThaiSan", TargetItems = "BenhVien", Enabled = false, Criteria = "NghiTapTrung=0")]
    public class TroCap_PHSKSauThaiSan : TroCap
    {
        // Fields...
        ...
        public TroCap_PHSKSauThaiSan(Session session) : base(session) { }

        public override void AfterConstruction()
        {
            base.AfterConstruction();

            LoaiTroCap = "Trợ cấp phụ hồi sức khỏe sau thai sản";
        }
        ...
        private void TinhLuyKe()
        {
            ...
            object luyKe = Session.Evaluate<TroCap_PHSKSauThaiSan>(CriteriaOperator.Parse("SUM(LuyKeDauNam)"), go);
            if (luyKe != null)
        ...
        protected override void TinhSoTien()
        {
            ...
            {
                if (NghiTaiGiaDinh > 0)
                    tongSoTien += 25 * luongCoBan * NghiTaiGiaDinh;
                if (NghiTapTrung > 0)
