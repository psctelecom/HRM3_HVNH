...
{
    [NonPersistent]
    [ModelDefault("Caption", "Danh sách cán bộ nghỉ thai sản")]
    public class TimKiemCanBoNghiThaiSan : TruongBaseObject
    {
        ...
        public XPCollection<DanhSachCanBoNghiThaiSan> ListCanBo { get; set; }

        public TimKiemCanBoNghiThaiSan(Session session)
            : base(session)
        { }
        ...
        public override void AfterConstruction()
        {
            ...
            //
            ListCanBo = new XPCollection<DanhSachCanBoNghiThaiSan>(Session, false);
            //
        ...
        public void LoadData()
        {
                ...
                TuNgay <= DenNgay)
            {
                ...
                //
                DataTable dt = DataProvider.GetDataTable("spd_TimKiemDanhSachNghiThaiSan", CommandType.StoredProcedure, param);
                {
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        DanhSachCanBoNghiThaiSan DanhSach;
                        foreach (DataRow item in dt.Rows)
                        {
                            DanhSach = new DanhSachCanBoNghiThaiSan(Session);
