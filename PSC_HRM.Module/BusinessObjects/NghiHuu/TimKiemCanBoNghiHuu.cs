...
{
    [NonPersistent]
    [ModelDefault("Caption", "Danh sách cán bộ nghỉ hưu")]
    public class TimKiemCanBoNghiHuu : TruongBaseObject
    {
        ...
        public XPCollection<DanhSachCanBoNghiHuu> ListCanBo { get; set; }

        public TimKiemCanBoNghiHuu(Session session)
            : base(session)
        { }
        ...
        public override void AfterConstruction()
        {
            ...
            //
            ListCanBo = new XPCollection<DanhSachCanBoNghiHuu>(Session, false);
            //
        ...
        public void LoadData()
        {
                ...
                TuNgay <= DenNgay)
            {
                ...
                //
                DataTable dt = DataProvider.GetDataTable("spd_TimKiemDanhSachNghiHuu", CommandType.StoredProcedure, param);
                {
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        DanhSachCanBoNghiHuu DanhSach;
                        foreach (DataRow item in dt.Rows)
                        {
                            DanhSach = new DanhSachCanBoNghiHuu(Session);
