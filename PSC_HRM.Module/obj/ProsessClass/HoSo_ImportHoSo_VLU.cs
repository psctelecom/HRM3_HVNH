...
namespace PSC_HRM.Module.Controllers
{
    ...
    class HoSo_ImportHoSo_VLU
    {
        ...
        public static bool Import(HoSo_Import obj, IObjectSpace obs, string filename, bool doTatCa)
        {
            ...
            using (DataTable dt = DataProvider.GetDataTable(filename, "[ThongTinNhanVien$A5:DR]"))
            {
                #region tblHoSO
                const int sttIdx = 0;
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        else
                        {
                            ...
                            #region Quốc tịch cột 24
                            {
                                ...
                                if (!string.IsNullOrEmpty(text))
                                {
                                    QuocGia quocTich = uow.FindObject<QuocGia>(CriteriaOperator.Parse("TenQuocGia like ?", text));
                                    if (quocTich == null)
                                    {
                            ...
                            {
                                String tenNgachLuong = dr[ngachLuongIdx].ToString().RemoveEmpty();
                                if (!string.IsNullOrEmpty(tenNgachLuong))
                                {
                                    NgachLuong ngach = null;
                                    ngach = uow.FindObject<NgachLuong>(CriteriaOperator.Parse(" TenNgachLuong like ?", tenNgachLuong));
                                    ...
                                    if (ngach != null)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(bacLuongText))
                                        {
                                            ...
                                            if (bacLuong == null)
                                            {
                                                String mucP1String = dr[P1Idx].ToString().Trim();
                                                String mucP3String = dr[P3Idx].ToString().Trim();
                                                decimal p1 = 0;
                                                decimal p3 = 0;
                                                bool kq1 = decimal.TryParse(mucP1String, out p1);
                                                bool kq3 = decimal.TryParse(mucP3String, out p3);
                                                if (kq1 == false)
                                                    errorLog.AppendLine(" + Thiếu mức lương BHXH(P1) hoặc không đúng định dạng số: " + mucP1String);
                                                if (kq3 == false)
                                                    errorLog.AppendLine(" + Thiếu thu nhập ăng thêm(P3) hoặc không đúng định dạng số: " + mucP3String);
                                                bacLuong = new BacLuong(uow);
                                                bacLuong.P1MucLuongDongBHXH = p1;
                                                bacLuong.P3ThuNhapTangThem = p3;
                                            ...
                                            if (bacLuong != null)
                                            {
                                                ...
                                                //nhanVien.NhanVienThongTinLuong.P1MucLuongDongBHXH = bacLuong.P1MucLuongDongBHXH;
                                                //nhanVien.NhanVienThongTinLuong.P3ThuNhapTangThem = bacLuong.P3ThuNhapTangThem;
                                            }
                                            else
                                            {
