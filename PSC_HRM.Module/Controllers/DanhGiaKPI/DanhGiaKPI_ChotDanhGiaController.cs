...
{
    public partial class DanhGiaKPI_ChotDanhGiaController : ViewController
    {
        public DanhGiaKPI_ChotDanhGiaController()
        {
            InitializeComponent();
            RegisterActions(components);
            HamDungChung.DebugTrace("DanhGiaKPI_ChotDanhGiaController");
        }
        ...
        private void simpleAction1_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            ...
            if (qldanhGia != null)
            {
                using (DialogUtil.AutoWait())
                {
                    ...
                    //
                    DataProvider.ExecuteNonQuery("dbo.spd_DanhGiaKPI_ChotDanhGia", System.Data.CommandType.StoredProcedure, param);

                    View.ObjectSpace.Refresh();
                }
                DialogUtil.ShowInfo("Chốt đánh giá thành công!");
            }
            else
            { DialogUtil.ShowError("Chưa chọn kỳ tính lương."); }
        }

        private void DanhGiaKPI_ChotDanhGiaController_Activated(object sender, EventArgs e)
        {
