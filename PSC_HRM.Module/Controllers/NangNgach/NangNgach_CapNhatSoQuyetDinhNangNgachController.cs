...
{
    public partial class NangNgach_CapNhatSoQuyetDinhNangNgachController : ViewController
    {
        public NangNgach_CapNhatSoQuyetDinhNangNgachController()
        {
            InitializeComponent();
            RegisterActions(components);
            HamDungChung.DebugTrace("NangNgach_CapNhatSoQuyetDinhNangNgachController");
        }
        private void NangNgach_CapNhatSoQuyetDinhNangNgachController_Activated(object sender, EventArgs e)
        {
                            ...
                            using (DataTable dt = DataProvider.GetDataTable(dialog.FileName, "[Sheet1$A1:B]"))
                            {
                                ...
                                if (dt != null && dt.Rows.Count > 0)
                                {
                                    foreach (DataRow item in dt.Rows)
                                    {
                                        ...
                                        { break; }
                                        //
                                        QuyetDinhNangNgach qd = uow.FindObject<QuyetDinhNangNgach>(CriteriaOperator.Parse("ListChiTietQuyetDinhNangNgach[ThongTinNhanVien.MaQuanLy=?]", item[idx_MaQuanLy].ToString().Trim()));
                                        if (qd != null)
                                        {
                                            if (!string.IsNullOrEmpty(item[idx_SoQuyetDinh].ToString().RemoveEmpty()) && String.IsNullOrEmpty(qd.SoQuyetDinh) && qd.ListChiTietQuyetDinhNangNgach.Count == 1)
                                            {
