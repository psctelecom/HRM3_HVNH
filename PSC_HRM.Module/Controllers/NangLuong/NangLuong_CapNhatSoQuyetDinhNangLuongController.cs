...
{
    public partial class NangLuong_CapNhatSoQuyetDinhNangLuongController : ViewController
    {
       
        public NangLuong_CapNhatSoQuyetDinhNangLuongController()
        {
            InitializeComponent();
            RegisterActions(components);
            HamDungChung.DebugTrace("NangLuong_CapNhatSoQuyetDinhNangLuongController");
        }
        private void NangLuong_CapNhatSoQuyetDinhNangLuongController_Activated(object sender, EventArgs e)
        {
                            ...
                            using (DataTable dt = DataProvider.GetDataTable(dialog.FileName, "[Sheet1$A1:B]"))
                            {
                                ...
                                if (dt != null && dt.Rows.Count > 0)
                                {
                                    foreach (DataRow item in dt.Rows)
                                    {
                                        ...
                                        { break; }
                                        //
                                        QuyetDinhNangLuong qd = uow.FindObject<QuyetDinhNangLuong>(CriteriaOperator.Parse("ListChiTietQuyetDinhNangLuong[ThongTinNhanVien.MaQuanLy=?]", item[idx_MaQuanLy].ToString().Trim()));
                                        if (qd != null)
                                        {
                                            if (!string.IsNullOrEmpty(item[idx_SoQuyetDinh].ToString().RemoveEmpty()) && String.IsNullOrEmpty(qd.SoQuyetDinh) && qd.ListChiTietQuyetDinhNangLuong.Count == 1)
                                            {
