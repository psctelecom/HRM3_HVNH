...
{
    public partial class TapSu_CapNhatSoQuyetDinhBoNhiemNgachController : ViewController
    {
        public TapSu_CapNhatSoQuyetDinhBoNhiemNgachController()
        {
            InitializeComponent();
            RegisterActions(components);
            HamDungChung.DebugTrace("TapSu_CapNhatSoQuyetDinhBoNhiemNgachController");
        }
        private void TapSu_CapNhatSoQuyetDinhBoNhiemNgachController_Activated(object sender, EventArgs e)
        {
                            ...
                            using (DataTable dt = DataProvider.GetDataTable(dialog.FileName, "[Sheet1$A1:B]"))
                            {
                                ...
                                if (dt != null && dt.Rows.Count > 0)
                                {
                                    foreach (DataRow item in dt.Rows)
                                    {
                                        ...
                                        { break; }
                                        //
                                        QuyetDinhBoNhiemNgach qd = uow.FindObject<QuyetDinhBoNhiemNgach>(CriteriaOperator.Parse("ListChiTietQuyetDinhBoNhiemNgach[ThongTinNhanVien.MaQuanLy=?]", item[idx_MaQuanLy].ToString().Trim()));
                                        if (qd != null)
                                        {
                                            if (!string.IsNullOrEmpty(item[idx_SoQuyetDinh].ToString().RemoveEmpty()) && String.IsNullOrEmpty(qd.SoQuyetDinh) && qd.ListChiTietQuyetDinhBoNhiemNgach.Count == 1)
                                            {
